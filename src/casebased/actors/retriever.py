...
from ..utils.sim import SimMapWrapper, get_metadata, similarities2ranking, unpack_sim
from ..components.similarity.types import (
    AnySimFunc,
    Float,
    JsonDict,
    RetrieverFunc,
...
class Result[K, V, S: Float]:
    """
    Result of a retrieval process.

    Args:
        steps: List of ResultStep objects
...
def mapply[QK, CK, V, S: Float](
    ...
    return {
        key: retrieve(casebase, value, retrievers, processes)
        for key, value in queries.items()
    }


def retrieve[K, V, S: Float](
    casebase: CaseBase,
    query: V,

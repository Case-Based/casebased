...
from ...utils.sim import SimSeqWrapper, get_metadata
from .types import (
    AnySimFunc,
    JsonDict,
    SimPairFunc,
...
"""
These functions are used to define similarity measures in a generic way.

The `table` function allows to import a similarity values from a table.
The `static_table` function is a more specific version of `table` that assumes the table is symmetric.
The `dynamic_table` function allows to import a similarity values from a table that depends on the input values.
The `equality` function returns 1.0 if the two values are equal, 0.0 otherwise.
The `static` function returns a constant value for all pairs.
The `transpose` function transforms a similarity function from one type to another.
"""

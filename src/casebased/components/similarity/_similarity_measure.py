...
from ...utils import SimSeqWrapper, get_metadata
from .types import (
    AggregatorFunc,
    AnnotatedFloat,
    AnySimFunc,
...
__all__ = ["similarity_measure", "AttributeValueSim"]

"""
These functions are used to define similarity measures based on attribute values.

The `attribute_value` function computes the similarity between two cases based on their attribute values.
Given are a mapping of attribute names to similarity functions and an aggregator function that combines the local similarities into a global similarity.
...
@dataclass(slots=True, frozen=True)
class similarity_measure[V, S: Float](
